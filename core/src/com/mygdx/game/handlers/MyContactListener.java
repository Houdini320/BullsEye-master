...
public class MyContactListener implements ContactListener {
    ...
    public void beginContact(Contact contact) {
        ...
        if (isSensorContact(fa, fb)){
            B2DBodyBuilder obstacle;
            Ball ball2;
            if(fa.getUserData()instanceof B2DBodyBuilder){
                obstacle = (B2DBodyBuilder) fa.getUserData();
                ball2 = (Ball) fb.getUserData();
            }
            else {
                obstacle = (B2DBodyBuilder) fb.getUserData();
    ...
    //SET THE CLASS IN THE WORLD
    //GameScreen.world.setMyContactListener(MyContactListener);
}
